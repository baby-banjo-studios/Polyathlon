            ...
            foreach (Racer racer in racers)
            {
                if (!racer.IsDead() && racer != owner)
                {
                    if (ValidateTarget(racer))
                    {
                        ...
                        if (racer.movementMode == MovementMode.Jetpacking || racer.movementMode == MovementMode.Gliding)
                        {
        ...
        private bool ValidateTarget(Racer target)
        {
            switch (targetMode)
            {
                ...
                case TargetMode.Airborne:
                    {
                        return (target.movementMode == MovementMode.Jetpacking || target.movementMode == MovementMode.Gliding) && !target.isGrounded();
                    }
                case TargetMode.AirborneIncludingGrounded:
                    {
                        return target.movementMode == MovementMode.Jetpacking || target.movementMode == MovementMode.Gliding;
                    }
                default:
                    {
                        return false;

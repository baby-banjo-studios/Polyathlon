...
namespace BabyBanjo.Polyathlon.Entities
{
    public class NPC : Racer
    {
        ...
        protected override void Start()
        {
            ...
            foreach (BaseMovement m in movementOptions)
            {
                m.enabled = false;
        ...
        protected override void Update()
        {
            ...
            base.Update();
            if (movementMode == MovementMode.Jetpacking)
            {
            ...
            else if (movementMode == MovementMode.Gliding)
            {
                if (!movement.Grounded)
                {
            ...
            else if (movementMode == MovementMode.Biking)
            {
                agent.acceleration = Mathf.Lerp(1.2f, 10f, rb.linearVelocity.magnitude / movement.maxSpeed);
            }
            else if (movementMode == MovementMode.Swimming || movementMode == MovementMode.GetOffTheBoat)
            {
            ...
            else if (movementMode == MovementMode.Wheeling)
            {
        ...
        void FixedUpdate()
        {
            if (!rb.isKinematic)
            {
                if (movementMode == MovementMode.Running || movementMode == MovementMode.Jetpacking && movement.Grounded)
                {
                ...
                else if (movementMode == MovementMode.Biking && movement.BoostSpeedScale == 1 && movement.PermanentSpeedScale == 1)
                {
        ...
        public void Land()
        {
            if (movementMode == MovementMode.Jetpacking || movementMode == MovementMode.Gliding)
            {
                SetNavMeshAgent(true);
                if (movementMode == MovementMode.Gliding)
                {
        ...
        public override void SetMovementMode(MovementMode mode, bool initial = false)
        {
            Debug.Log("Setting mode: " + mode);
            base.SetMovementMode(mode, initial);
            move = new Vector2(0, 0);
            if (mode == MovementMode.Swimming || mode == MovementMode.GetOffTheBoat)
            {
                SetNavMeshAgent(false);
            }
            else
        ...
        private IEnumerator FallMonitor()
        {
            ...
            while (movementMode == MovementMode.Wheeling)
            {
